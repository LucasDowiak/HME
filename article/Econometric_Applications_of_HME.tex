...
the context, a researcher may suspect the presence of some hidden heterogeneity. 
Rather than taking a firm stance on any concealed structure,
an HME setup ideally limits the work the researcher needs to do to specifying
a set of well-chosen conditioning variables $Z$ to feed through the gating
network.

multi-modal distributions
sections \ref{sec:SimpleExample} and \ref{sec:MincerWageEx}.


What pulls these different threads together is the uncertainty of the true
model specification. To guard against the maleffects of a misspecified
model on the estimated standard erros, we use a sandwich estimator for the
variance-covariance matrix.




latent structure is the Iris. A linear relationship exists within subgroups
of the data, but the researcher is unsure how the subgroups are delineated.

\cite{HuertaJiangTanner2003} use it the HME structure to test for correct 
model specifications.


Frequently, it's a belief that a latent structure exists in 

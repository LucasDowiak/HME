...
Just as for logistic regressions, we can calculate the marginal effect of the entire
gating network, providing a sense of what gating variables play a decisive role in 
directing input patterns to the appropriate expert regression. Starting with equation
(\ref{eq:gpath2}), we take the partial with respect to gating matrix $Z_{t}$.

\begin{equation}
  \delta^{m} \equiv \frac{\partial \gateprod{g^{0}}{P^{m}}}{\partial Z} = \frac{\partial g^{0, i} g^{i|0, j} \cdots g^{k|\cdots|j|i|0, m}}{\partial Z}
\end{equation}
...
\begin{equation} \label{eq:gate_marginal_effect}
  \frac{\partial g^{a, i}}{\partial Z} = g^{a, i}(\omega^{a, i} - \sum_{j} g^{a, j} \omega^{a, j}) = g^{a, i}(\omega^{a, i} - \mean{\omega}^{a})
\end{equation}
...
\begin{equation} \label{eq:marginal_effects}
  \delta^{m} = \gateprod{g^{0}}{P^{m}} (\omega^{0, i} + \omega^{i|0, j} + \cdots + \omega^{k|\cdots|j|i|0, m} - (\mean{\omega}^{0} + \mean{\omega}^{i|0} + \cdots + \mean{\omega}^{k|\cdots|j|i|0}))
\end{equation}
